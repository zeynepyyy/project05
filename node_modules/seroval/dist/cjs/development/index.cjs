...
function createMapNode(ctx, id, k, v, s) {
  return createSerovalNode(
    8 /* Map */,
    id,
    NIL,
...
async function parseItems(ctx, current) {
  const nodes = [];
...
async function parseArray(ctx, id, current) {
  return createArrayNode(id, current, await parseItems(ctx, current));
}
async function parseProperties(ctx, properties) {
...
async function parsePlainObject(ctx, id, current, empty) {
...
async function parseBoxed(ctx, id, current) {
  return createBoxedNode(id, await parseAsync(ctx, current.valueOf()));
}
async function parseTypedArray(ctx, id, current) {
...
async function parseBigIntTypedArray(ctx, id, current) {
...
async function parseDataView(ctx, id, current) {
  return createDataViewNode(id, current, await parseAsync(ctx, current.buffer));
}
async function parseError(ctx, id, current) {
...
async function parseAggregateError(ctx, id, current) {
  const options = getErrorOptions(current, ctx.base.features);
...
async function parseMap(ctx, id, current) {
  const keyNodes = [];
  const valueNodes = [];
...
async function parseSet(ctx, id, current) {
  const items = [];
...
async function parsePlugin(ctx, id, current) {
...
async function parsePromise(ctx, id, current) {
...
function parseStreamHandle(id, current, resolve, reject) {
  const sequence = [];
...
async function parseStream(ctx, id, current) {
  return createStreamConstructorNode(
    id,
    parseSpecialReference(ctx.base, 4 /* StreamConstructor */),
    await new Promise(parseStreamHandle.bind(ctx, id, current))
  );
}
async function parseObjectAsync(ctx, id, current) {
  ...
  switch (currentClass) {
    ...
    case Float32Array:
    case Float64Array:
      return parseTypedArray(ctx, id, current);
    case DataView:
      return parseDataView(ctx, id, current);
    case Map:
      return parseMap(ctx, id, current);
    case Set:
      return parseSet(ctx, id, current);
    default:
      break;
  }
  if (currentClass === Promise || current instanceof Promise) {
    return parsePromise(ctx, id, current);
  }
  ...
  if (currentFeatures & 1 /* AggregateError */ && typeof AggregateError !== "undefined" && (currentClass === AggregateError || current instanceof AggregateError)) {
    return parseAggregateError(ctx, id, current);
  }
...
async function parseFunctionAsync(ctx, current) {
...
async function parseAsync(ctx, current) {
  switch (typeof current) {
    ...
    case "object": {
      if (current) {
        const ref = getReferenceNode(ctx.base, current);
        return ref.type === 0 ? await parseObjectAsync(ctx, ref.value, current) : ref.value;
      }
      return NULL_NODE;
    }
    case "symbol":
...
var DeserializePluginContext = class {
  ...
  deserialize(node) {
    return deserialize(this._p, node);
  }
};
  ...
  if (len) {
    ...
    for (let i = 0, key; i < len; i++) {
      ...
      if (typeof key === "string") {
        ...
        result[deserialize(ctx, key)] = deserialize(ctx, vals[i]);
      }
    }
  }
  return result;
...
function deserializeTypedArray(ctx, node) {
  ...
  const source = deserialize(ctx, node.f);
  const result = assignIndexedValue(
    ctx,
    node.i,
...
function deserializePlugin(ctx, node) {
  const currentPlugins = ctx.base.plugins;
  if (currentPlugins) {
...
function deserializePromiseResolve(ctx, node) {
...
function deserializePromiseReject(ctx, node) {
  const deferred = ctx.base.refs.get(node.i);
  if (deferred) {
...
function deserializeIteratorFactoryInstance(ctx, node) {
  deserialize(ctx, node.a[0]);
  const source = deserialize(ctx, node.a[1]);
  return sequenceToIterator(source);
}
function deserializeAsyncIteratorFactoryInstance(ctx, node) {
  deserialize(ctx, node.a[0]);
  const source = deserialize(ctx, node.a[1]);
  return streamToAsyncIterable(source);
}
function deserializeStreamConstructor(ctx, node) {
...
function deserializeStreamNext(ctx, node) {
  const deferred = ctx.base.refs.get(node.i);
  if (deferred) {
...
function deserializeStreamThrow(ctx, node) {
  const deferred = ctx.base.refs.get(node.i);
  if (deferred) {
...
function deserializeStreamReturn(ctx, node) {
  const deferred = ctx.base.refs.get(node.i);
  if (deferred) {
...
function deserializeIteratorFactory(ctx, node) {
  deserialize(ctx, node.f);
  return void 0;
}
function deserializeAsyncIteratorFactory(ctx, node) {
  deserialize(ctx, node.a[1]);
  return void 0;
}
function deserialize(ctx, node) {
  switch (node.t) {
    case 2 /* Constant */:
...
function parseItems2(ctx, current) {
  const nodes = [];
  for (let i = 0, len = current.length; i < len; i++) {
    if (i in current) {
...
function parseArray2(ctx, id, current) {
  return createArrayNode(id, current, parseItems2(ctx, current));
}
function parseProperties2(ctx, properties) {
  const entries = Object.entries(properties);
  const keyNodes = [];
  const valueNodes = [];
  for (let i = 0, len = entries.length; i < len; i++) {
...
function parsePlainObject2(ctx, id, current, empty) {
  return createObjectNode(id, current, empty, parseProperties2(ctx, current));
}
function parseBoxed2(ctx, id, current) {
  return createBoxedNode(id, parseSOS(ctx, current.valueOf()));
}
function parseTypedArray2(ctx, id, current) {
  return createTypedArrayNode(id, current, parseSOS(ctx, current.buffer));
}
function parseBigIntTypedArray2(ctx, id, current) {
  return createBigIntTypedArrayNode(id, current, parseSOS(ctx, current.buffer));
}
function parseDataView2(ctx, id, current) {
  return createDataViewNode(id, current, parseSOS(ctx, current.buffer));
}
function parseError2(ctx, id, current) {
  const options = getErrorOptions(current, ctx.base.features);
  return createErrorNode(
    id,
    current,
    options ? parseProperties2(ctx, options) : NIL
  );
}
function parseAggregateError2(ctx, id, current) {
  const options = getErrorOptions(current, ctx.base.features);
  return createAggregateErrorNode(
    id,
    current,
...
function parseMap2(ctx, id, current) {
  const keyNodes = [];
  const valueNodes = [];
  for (const [key, value] of current.entries()) {
...
function parseSet2(ctx, id, current) {
  const items = [];
  for (const item of current.keys()) {
...
function parseStream2(ctx, id, current) {
...
function handlePromiseSuccess(id, data) {
  if (this.state.alive) {
...
function handlePromiseFailure(id, data) {
  if (this.state.alive) {
    const parsed = parseWithError(this, data);
    if (parsed) {
      onParse(
...
function parsePromise2(ctx, id, current) {
...
function parsePluginSync(ctx, id, current, currentPlugins) {
  for (let i = 0, len = currentPlugins.length; i < len; i++) {
    const plugin = currentPlugins[i];
...
function parsePluginStream(ctx, id, current, currentPlugins) {
  for (let i = 0, len = currentPlugins.length; i < len; i++) {
    const plugin = currentPlugins[i];
...
function parsePlugin2(ctx, id, current) {
  const currentPlugins = ctx.base.plugins;
  if (currentPlugins) {
    return ctx.type === 1 /* Sync */ ? parsePluginSync(ctx, id, current, currentPlugins) : parsePluginStream(ctx, id, current, currentPlugins);
  }
  return void 0;
}
function parseObjectPhase2(ctx, id, current, currentClass) {
  switch (currentClass) {
    case Object:
    ...
    case TypeError:
    case URIError:
      return parseError2(ctx, id, current);
    case Number:
    case Boolean:
    case String:
    case BigInt:
      return parseBoxed2(ctx, id, current);
    case ArrayBuffer:
      return createArrayBufferNode(
        ctx.base,
        id,
    ...
    case Float32Array:
    case Float64Array:
      return parseTypedArray2(ctx, id, current);
    case DataView:
      return parseDataView2(ctx, id, current);
    case Map:
      return parseMap2(ctx, id, current);
    case Set:
      return parseSet2(ctx, id, current);
    default:
      break;
  }
  if (currentClass === Promise || current instanceof Promise) {
    return parsePromise2(ctx, id, current);
  }
  const currentFeatures = ctx.base.features;
  if (currentFeatures & 16 /* BigIntTypedArray */) {
    switch (currentClass) {
  ...
  if (currentFeatures & 1 /* AggregateError */ && typeof AggregateError !== "undefined" && (currentClass === AggregateError || current instanceof AggregateError)) {
    return parseAggregateError2(ctx, id, current);
  }
  if (current instanceof Error) {
    return parseError2(ctx, id, current);
  }
  if (SYM_ITERATOR in current || SYM_ASYNC_ITERATOR in current) {
    return parsePlainObject2(ctx, id, current, !!currentClass);
  }
  throw new SerovalUnsupportedTypeError(current);
}
function parseObject(ctx, id, current) {
  if (Array.isArray(current)) {
    return parseArray2(ctx, id, current);
  }
  if (isStream(current)) {
    return parseStream2(ctx, id, current);
  }
  const currentClass = current.constructor;
  if (currentClass === OpaqueReference) {
    return parseSOS(
  ...
  const parsed = parsePlugin2(ctx, id, current);
  if (parsed) {
    return parsed;
  }
  return parseObjectPhase2(ctx, id, current, currentClass);
}
function parseFunction(ctx, current) {
  const ref = getReferenceNode(ctx.base, current);
  if (ref.type !== 0 /* Fresh */) {
    return ref.value;
  }
  const plugin = parsePlugin2(ctx, ref.value, current);
  if (plugin) {
    return plugin;
  }
  throw new SerovalUnsupportedTypeError(current);
}
function parseSOS(ctx, current) {
  switch (typeof current) {
    case "boolean":
      return current ? TRUE_NODE : FALSE_NODE;
    case "undefined":
    ...
    case "object": {
      if (current) {
        const ref = getReferenceNode(ctx.base, current);
        return ref.type === 0 /* Fresh */ ? parseObject(ctx, ref.value, current) : ref.value;
      }
      return NULL_NODE;
    }
    case "symbol":
...
function parseWithError(ctx, current) {
  try {
    return parseSOS(ctx, current);
  } catch (err) {
    onError(ctx, err);
